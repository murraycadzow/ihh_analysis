...
p = ggplot(data = rsb, aes(x=POSITION, y=Std_iHH), ylim=c(min(rsb$Std_iHH -0.5), max(rsb$Std_iHH + 0.5)))  + geom_point(shape = 16, colour= alpha("black", 1/5)) + facet_grid(POP2~ POP1) + geom_hline(aes(yintercept= -3.29, colour ="Threshold"))  + geom_hline(aes(yintercept= 3.29, colour ="Threshold")) + xlab(paste0("Chromosome ", i, " Position (Mbp)")) + ylab("Rsb/iHS") + ggtitle("Rsb and iHS (Genetic)") + geom_segment(data= subset(gaps, gaps$chrom == i),aes(x = chromStart, xend=chromEnd, y=min(rsb$Std_iHH-0.5), yend=min(rsb$Std_iHH-0.5), color="gap"),size=4)#+geom_point(data=ihs, aes(y = Std_iHH, x=POSITION,colour="PPARGC1A iHS"))
}
...
annotate=function(ihs){
  ...
  genes_new=c("TRIM46", "INHBB", "ORC4L", "SFMBT1","TMEM171", 
              ...
              "HLF", "BCAS3", "QRICH2")
  #plot(ann1 %>% filter(gene %in% genes_new) %>% group_by(gene) %>% ggplot(. , aes(x=gene, y=rank)) + 
...
for( pop2 in c("AXIOM","OMNI","CEU","CHB","CHS","GBR","YRI")){
annotate(as.data.frame(rsb %>% filter(pop2 == POP2)))
}
```
