...
annotate=function(ihs){
  ...
  plot(ann1 %>% filter(gene %in% genes) %>% group_by(gene) %>% ggplot(. , aes(x=gene, y=rank), ylim=c(0,6e6)) + 
  ...
  genes_new=c("TRIM46", "INHBB", "ORC4L", "SFMBT1","TMEM171", 
              ...
              "HLF", "BCAS3", "QRICH2")
  plot(ann1 %>% filter(gene %in% genes_new) %>% group_by(gene) %>% ggplot(. , aes(x=gene, y=rank), ylim=c(0,6e6)) + 
