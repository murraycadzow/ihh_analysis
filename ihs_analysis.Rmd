...
for( POP in c("AXIOM","OMNI","CEU","CHB","CHS","GBR","YRI")){
  ...
  #print(names(ihs))
  #print(head(ihs))
  manhattan(x = ihs[!is.na(ihs$iHS),], chr = "CHR", bp = "POSITION", p = "Pvalue")
  s = ihs %>% group_by(CHROM) %>% summarise(m=mean(iHS), sd(iHS), min(iHS), max(iHS), q1 = quantile(iHS, 0.01), q2 = quantile(iHS, 0.99))
